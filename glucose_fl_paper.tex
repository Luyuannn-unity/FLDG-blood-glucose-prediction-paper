...
\paragraph{Adapting to a new cohort: federated pretraining vs.\ from scratch}

When a federated model is fine-tuned on the previously unseen ReplaceBG cohort,
it consistently outperforms a model trained on the same ReplaceBG data from
random initialisation (Table~\ref{tab:replacebg-adapt}). All three federated
starting points improve on from-scratch training by roughly $0.3$--$0.4$~mg/dL
RMSE at the 30-minute horizon, with FedProx the strongest at
$20.98 \pm 0.06$~mg/dL versus $21.38 \pm 0.09$~mg/dL from scratch.
FedAvg and MLDG land together at $21.04$~mg/dL. Because both arms see identical
ReplaceBG data and training budget, the improvement is attributable to the
federation-pretrained initialisation: training across the four heterogeneous
client cohorts yields weights that transfer to a new site faster and to a lower
error floor than learning that site in isolation.

\begin{table}[!ht]
\centering
\captionsetup{justification=centering,singlelinecheck=on}
\caption{\bf Adapting to the held-out ReplaceBG cohort.}
\label{tab:replacebg-adapt}
\begin{tabular}{lcc}
\thickhline
adaptation arm & per-seed RMSE (42,43,44,46,47) & mean $\pm$ sd \\
\hline
FedProx $\rightarrow$ fine-tune & 20.96, 20.95, 20.99, 20.92, 21.08 & \textbf{20.98 $\pm$ 0.06} \\
FedAvg $\rightarrow$ fine-tune  & 21.06, 21.06, 21.05, 21.05, 20.96 & 21.04 $\pm$ 0.04 \\
MLDG $\rightarrow$ fine-tune    & 21.06, 21.03, 21.03, 21.15, 20.93 & 21.04 $\pm$ 0.07 \\
ReplaceBG from scratch          & 21.49, 21.34, 21.47, 21.28, 21.32 & 21.38 $\pm$ 0.09 \\
\thickhline
\end{tabular}
\begin{center}
{\small RMSE (mg/dL) at the 30-minute horizon on ReplaceBG's held-out patients,
over 5 seeds. Federated models are fine-tuned on ReplaceBG's training patients;
the from-scratch baseline trains the same architecture on the same data from
...
The adaptation experiment speaks directly to how a new site would actually adopt
one of these models. Pure zero-shot transfer is the hardest case --- a cohort
that contributed no data at all --- but most newly onboarded sites can collect at
least some of their own CGM data before deployment. In that more realistic
setting, a federation-pretrained model is a strictly better starting point than
training on the new cohort alone: fine-tuning FedAvg, FedProx, or MLDG on
ReplaceBG reaches a lower error floor than the same architecture trained from
scratch on identical data, and does so by a margin several times the between-seed
noise. The gain is not explained by extra data or compute, since both arms are
matched on each; it reflects a more transferable initialisation learned by
training across four heterogeneous cohorts. This reframes the value of the
federation: even for a site that never joins the training consortium, the global
model is worth taking as a warm start rather than starting over.

That the best starting point is FedProx rather than MLDG is consistent with the
fine-tuning objective. MLDG optimises explicitly for zero-shot generalisation to
held-out domains, whereas here the model is allowed to adapt; once a few epochs of
in-domain data are available, the meta-learned initialisation no longer has a
clear advantage, and the simpler drift-controlled FedProx solution transfers at
least as well. The three federated arms nonetheless cluster tightly
($20.98$--$21.04$~mg/dL), so the practical message is robust to the choice of
federated strategy: pretraining across cohorts helps, and the specific aggregation
rule matters less once the model is fine-tuned.
