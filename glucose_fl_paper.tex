...
\paragraph{The hardest cohort benefits most from personalisation.} ARISES, the
smallest and highest-glucose cohort, is the hardest client for every strategy
($\approx 22$ mg/dL). It is the one client where the global strategies are
\emph{not} best: Ditto ($\mu{=}0.01$, $21.98$) and the dedicated ARISES-only
model ($22.04$) edge out FedAvg and MLDG, indicating that a small, distinctive
cohort gains from a personalised or dedicated model in-domain --- the same
dedicated model that, as noted above, generalises worst of all out-of-domain.

\paragraph{Adding bolus insulin does not improve forecasting.} The two-channel
CGM\,+\,bolus variant matches the CGM-only model to within seed noise and is, if
anything, marginally worse. The held-in averages are essentially unchanged
(MLDG $20.05$ vs.\ $19.99$; FedAvg $20.15$ vs.\ $20.09$ mg/dL) and the OOD scores
are within $0.1$ mg/dL (MLDG $21.34$ in both; FedAvg $21.48$ vs.\ $21.43$). The
ranking of strategies is identical in both input regimes. With bolus events
non-zero at only $\approx 1\%$ of timesteps, the sparse insulin signal adds no
measurable forecasting value at the 30-minute horizon over CGM history alone.
