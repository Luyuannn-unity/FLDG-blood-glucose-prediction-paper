...
| 7 | **Data availability — remaining gaps**: (a) add a **LICENSE** to `github.com/Luyuannn-unity/fldg-glucose` (no licence = nobody may legally run it; MIT or Apache-2.0 are the usual picks) and ideally mint a **Zenodo DOI**; (b) accession numbers/URLs for the public datasets (MetaboNet, ReplaceBG, BrisT1D, Flair); (c) access route + justification for the proprietary ABC4D and ARISES (PLOS scrutinises "proprietary" — name a data-access contact, not "on request"); (d) IRB/consent statement for secondary use | Code repo is LIVE (pushed 2026-08-11, code-only, scanned for secrets/data first) and the paper now cites the real URL. |
