...
| 7 | **Data availability — fill the placeholders** (added 2026-08-11): replace `github.com/PLACEHOLDER/fldg-glucose` with the real repo (ideally + Zenodo DOI); add accession numbers/URLs for the public datasets (MetaboNet, ReplaceBG, BrisT1D, Flair); state the access route for the proprietary ABC4D and ARISES; add the IRB/consent statement for secondary use | The section now says: HUPA-UCM/T1D-UOM/ReplaceBG public via MetaboNet, BrisT1D/Flair public from original repositories, ABC4D/ARISES proprietary under data-sharing arrangements. ⚠️ PLOS scrutinises "proprietary": expect to justify why ABC4D/ARISES cannot be shared and name a data-access contact. Desk-reject risk if placeholders survive to submission. |
