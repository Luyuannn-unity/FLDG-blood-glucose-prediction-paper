...
|---|---|---|
| 1 | **Decide on the 7 skipped seed_46 arms** (`apfl`, `decoupled`, `ditto_mu0.01/0.1/1.0`, `single_ABC4D`, `single_HUPA-UCM`): author confirms seed 46 was *skipped* for these, not lost. Either (a) run these 7 arms at seed 46 to make every row 5-seed, or (b) keep the caption's 4-seed disclosure | Table verified 2026-08-11: every cell in tab:main reproduces exactly from the on-disk CSVs. Global rows = 5 seeds; APFL/decoupled/Ditto rows = 4 seeds (42,43,44,47) in all cells; Local = 4 seeds for HUPA-UCM and ABC4D, 5 for ARISES and T1D-UOM. The caption discloses this honestly, so (b) is defensible; (a) is cleaner and cheap (7 single-seed runs). If running: save checkpoints + per-window predictions. |
