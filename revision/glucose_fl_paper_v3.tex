...
small relative to CGM sensor error, so the practical value is reduced deployment
risk rather than a large accuracy jump, and the federated strategies differed
only within seed-to-seed variation. Finetuning the federated model on a new site
...
patients, whereas the federated models did not, on the one held-out cohort we
...
separately. ARISES is both the highest-glucose and the smallest cohort. Patient
counts are the usable subsets after quality filtering and are smaller than the
totals enrolled in the original dataset releases.}
\end{center}
\end{table}
...
original releases. The code for the federated system and the analysis, together
...
is the \emph{unweighted} mean of the four held-in client means (cohorts differ in
size, so it is not a window-weighted average). Local held-in is each cohort's own
...
\paragraph{The federated strategies differ only within seed-to-seed variation.}
Among the strategies that produce a deployable global model, MLDG has the
numerically lowest held-in average and OOD error and is numerically best on two
of the four clients, but the margins ($\approx 0.1$ mg/dL held-in and OOD) are
smaller than the across-seed standard deviations, we ran no significance test,
and the OOD comparison mixes 5-seed and 4-seed means (Table~\ref{tab:main}
...
single-cohort failure mode. FedProx was numerically \emph{weakest} of the three
global strategies here (highest RMSE both held-in and OOD), but it was run at a
...
\paragraph{Adding bolus insulin does not help at this scale.} In the preliminary
two-channel variant, adding the log-bolus input changed 30-minute RMSE by less
than about $0.05$ mg/dL on average across strategies --- within seed-to-seed
variation and in no consistent direction --- so we do not promote it to a main
result and draw no conclusion about exogenous inputs beyond this null (this check
used fewer seeds than the main experiment).
...
strategies differed only within seed-to-seed variation. Beyond the benchmark, we
showed that a federation of this kind runs across physically separate machines:
the same protocol trains FedAvg, FedProx, and MLDG over the public internet, with
no raw data leaving any site --- a working artifact, not a simulation.
...
ReplaceBG ($28.51 \pm 6.85$ mg/dL) and drove most of the local baseline's high
OOD variance
