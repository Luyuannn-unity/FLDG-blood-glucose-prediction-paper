...
\emph{OOD test sets.} Each held-out cohort is split by patient in the same way as
the clients, and we score OOD performance on its held-out \emph{test} patients
only. For ReplaceBG this is a 21-patient test split. We use that split rather than
the whole cohort deliberately: it is the same set of patients on which the
ReplaceBG-from-scratch and the finetuned models are evaluated (see below), so the
zero-shot, from-scratch, and finetuned arms are all compared on an identical test
set. Because no ReplaceBG patient contributes to federated training, these test
patients are unseen in the strong sense --- unseen cohort \emph{and} unseen
individual. All runs use early stopping on average validation MSE (patience~5 over a
...
$\mu$, and APFL may be under-tuned. \emph{(v) Horizon.} We report the 30- and
