...
in simulation, with the whole federation executed as processes or dataset shards
on a single machine.
...
simulated rather than deployed: the usual setup treats individual patients as
clients and executes the whole federation as processes or dataset shards on a
single machine
\cite{piao2023graph,piao2026gluadfl,darpit2025fedglu,defalco2023federated}. The
pattern is not specific to glucose --- of 612 federated healthcare studies only
$5.2\%$ report a real-life application \cite{teo2024federated}, and of 107
implementation studies only ten involve real-world distributed clinical settings
\cite{li2025challenges}. The exception is Sun et al.\
\cite{sun2025multicontinental}, who deploy a blockchain-enabled federated glucose
model across five cities on three continents, though on in-silico subjects from
the UVA/Padova simulator rather than real patient records.
Running a federation for real additionally requires networked,
...
holds the global model but no data, and clients that each wrap one dataset,
...
the University of Oxford --- with one client per institution, each training only
on one dataset and exchanging nothing but model weights over the public
internet. This exercises the conditions a federation faces across separate
administrative domains --- separate hardware, network round-trips between
rounds, no shared filesystem --- rather than the single-host shortcut that
almost all federated glucose work takes. What we do
