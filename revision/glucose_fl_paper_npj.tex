...
%  npj Digital Medicine variant, branched from glucose_fl_paper_working.tex
%  (identical to v30) on 2026-09-09. The working file stays the PLOS version.
%  Done here so far: title (<=15 words, no punctuation) and abstract (<=150
%  words). Still to do for npj: section order (Intro, Results, Discussion,
%  Methods), drop Author summary, strip Discussion \paragraph headers, split
%  the availability subsection, add Acknowledgements / Author contributions /
%  Competing interests, Nature reference style, "Fig." labels.
%  Layout below is still the PLOS class; npj applies no template at submission.
%
...
% npj: title up to 15 words, no punctuation (the colon had to go). 14 words as
% written; hyphenated compounds count as one. Alternatives considered:
%   Federated meta-learning for blood glucose forecasting across sites and a live four-institution deployment
%   Federated and meta-learned blood glucose forecasting across sites benchmarked and deployed live at four institutions
\begin{flushleft}
{\Large
\textbf{Benchmarking federated and meta-learned blood glucose forecasting across sites with a live four-institution deployment}
}
\newline
\\
...
\\
\textbf{5} FLock.io, London, United Kingdom
\\
...
* ken.li@ucl.ac.uk
% NOTE (author): (a) Taiyu Zhu confirmed 2026-09-09: KCL only, Oxford removed;
% (b) FLock.io line matches the affiliation printed in arXiv:2410.17933;
% (c) add department for Manchester if wanted;
...
% npj: abstract up to 150 words, no subheadings. 148 words as written. The
% 295-word PLOS abstract is in glucose_fl_paper_working.tex.
\section*{Abstract}
Blood glucose forecasting models warn people with type~1 diabetes (T1D) of
excursions but often transfer poorly to new sites, and the data that would fix
this cannot lawfully be pooled. Federated learning trains a shared model
without moving raw records, but its strategies for glucose forecasting are
rarely compared, seldom tested on unseen cohorts, and almost always simulated.
We ran a live federation across four universities, each holding one T1D cohort
(held-in), with three further cohorts as out-of-distribution tests, and
benchmarked local training against federated averaging, FedProx, meta-learning
for domain generalisation (MLDG) and two personalised methods. MLDG beat local
training held-in ($20.25$ vs $20.49$ mg/dL RMSE at 30 minutes), came within
$0.09$ mg/dL of a pooled centralised model, and was the best federated model
on all three unseen cohorts. Finetuning it beat training a new site from
scratch, with only $10\%$ of patients on the larger cohorts.
...
worldwide~\cite{gregory2022t1dindex}, who must continually balance
...
their evaluation~\cite{sergazinov2024glucobench}. But glucose dynamics differ
between populations~\cite{farahmand2025glumind}, demographics and devices
differ from site to site, and a model can degrade on populations it did not
see in training.
...
backbone beats a bank of deep-learning baselines in-cohort~\cite{zhu2026glullm}, but both
...
Newcastle University, and King's College London), exchanging nothing
...
Manchester, Newcastle University, and King's College London), with one client
...
30-minute horizon, and use it as the primary metric for three reasons.
...
As a clinical check on the same forecasts we also report the Clarke error
grid \cite{clarke1987} at 30 minutes (Table~\ref{tab:clarke}), computed on
the same test windows as the RMSE with the standard zone rules. Zone A holds
forecasts within $20\%$ of the reference value, and forecasts below 70 mg/dL
when the reference is also below 70 mg/dL. Zones D and E hold forecasts that
would miss a hypoglycaemic or hyperglycaemic event or prompt the wrong
treatment. We report the share of windows in zone A and in zones D and E
together.
...
meaningful, we compare their per-seed RMSE, or per-seed zone share, with a
two-sided paired $t$-test across the five seeds. We apply the same test to
every comparison we report:
...
\paragraph{The ordering holds on the Clarke error grid.} Table~\ref{tab:clarke}
scores the same 30-minute forecasts by their clinical consequence
\cite{clarke1987}. MLDG has the largest share of clinically accurate
forecasts (zone A) of the three global strategies in every column. Its gain
over local training is $0.9$ points held-in ($p = 0.058$, four of five
seeds) and $0.3$ to $0.9$ points on the OOD cohorts ($p \le 0.043$, five of
five seeds), while FedAvg and FedProx do not beat local training held-in.
MLDG also has the fewest clinically dangerous forecasts (zones D and E) of
the three in every column, $3.08\%$ against $3.57\%$ for local training
held-in ($p = 0.028$). HUPA-UCM is the one cohort where the local model does
better. The centralised baseline has the highest zone A share in every
column ($p = 0.014$ held-in) but is not significantly better than MLDG on
the dangerous zones ($p \ge 0.25$). At 60 minutes MLDG still has the fewest
dangerous forecasts of the three in every column, and its zone A gain over
local training is significant held-in ($0.8$ points, $p = 0.015$).

\begin{table}[!ht]
\centering
\captionsetup{justification=centering,singlelinecheck=on}
\caption{\bf Clarke error grid at the 30-minute horizon (\% of test windows).}
\label{tab:clarke}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccccccc}
\thickhline
...
strategy & HUPA-UCM & ABC4D & ARISES & T1D-UOM & avg & ReplaceBG & BrisT1D & Flair \\
\hline
\multicolumn{9}{l}{\emph{zone A, clinically accurate (higher is better)}} \\
Local (per cohort) & $87.60 \pm 0.56$ & $83.86 \pm 0.37$ & $79.43 \pm 1.30$ & $84.29 \pm 0.89$ & $83.80 \pm 0.51$ & $85.40 \pm 0.36$ & $82.44 \pm 0.15$ & $82.74 \pm 0.15$ \\
FedAvg & $87.21 \pm 0.45$ & $83.83 \pm 0.73$ & $80.33 \pm 0.32$ & $85.43 \pm 0.23$ & $84.20 \pm 0.41$ & $86.01 \pm 0.21$ & $82.65 \pm 0.16$ & $82.89 \pm 0.14$ \\
FedProx & $86.62 \pm 0.55$ & $83.54 \pm 0.70$ & $80.01 \pm 0.60$ & $85.24 \pm 0.46$ & $83.85 \pm 0.54$ & $85.79 \pm 0.32$ & $82.48 \pm 0.33$ & $82.79 \pm 0.26$ \\
MLDG & $\mathbf{87.87 \pm 0.39}$ & $\mathbf{84.52 \pm 0.60}$ & $\mathbf{80.65 \pm 0.32}$ & $\mathbf{85.81 \pm 0.25}$ & $\mathbf{84.71 \pm 0.37}$ & $\mathbf{86.28 \pm 0.21}$ & $\mathbf{82.85 \pm 0.14}$ & $\mathbf{83.04 \pm 0.14}$ \\
Centralised & $88.48 \pm 0.44$ & $85.05 \pm 0.60$ & $81.65 \pm 0.16$ & $86.63 \pm 0.10$ & $85.46 \pm 0.25$ & $86.63 \pm 0.05$ & $82.89 \pm 0.21$ & $83.38 \pm 0.22$ \\
\hline
\multicolumn{9}{l}{\emph{zones D and E, clinically dangerous (lower is better)}} \\
Local (per cohort) & $\mathbf{2.06 \pm 0.40}$ & $5.41 \pm 0.17$ & $3.87 \pm 0.28$ & $2.93 \pm 0.26$ & $3.57 \pm 0.17$ & $2.50 \pm 0.16$ & $1.12 \pm 0.04$ & $1.42 \pm 0.05$ \\
FedAvg & $3.00 \pm 0.36$ & $5.23 \pm 0.51$ & $3.23 \pm 0.18$ & $2.30 \pm 0.17$ & $3.44 \pm 0.30$ & $2.17 \pm 0.17$ & $1.05 \pm 0.05$ & $1.31 \pm 0.06$ \\
FedProx & $3.28 \pm 0.17$ & $5.51 \pm 0.32$ & $3.41 \pm 0.12$ & $2.37 \pm 0.10$ & $3.64 \pm 0.17$ & $2.30 \pm 0.10$ & $1.12 \pm 0.02$ & $1.39 \pm 0.03$ \\
MLDG & $2.60 \pm 0.25$ & $\mathbf{4.60 \pm 0.41}$ & $\mathbf{3.05 \pm 0.16}$ & $\mathbf{2.07 \pm 0.15}$ & $\mathbf{3.08 \pm 0.24}$ & $\mathbf{2.01 \pm 0.13}$ & $\mathbf{1.02 \pm 0.05}$ & $\mathbf{1.26 \pm 0.06}$ \\
Centralised & $2.37 \pm 0.52$ & $4.42 \pm 0.63$ & $2.93 \pm 0.27$ & $1.90 \pm 0.25$ & $2.91 \pm 0.41$ & $1.94 \pm 0.23$ & $1.00 \pm 0.09$ & $1.27 \pm 0.10$ \\
\thickhline
\end{tabular}}
\begin{center}
{\small Mean $\pm$ standard deviation across 5 random seeds, on the same
test windows and forecasts as Table~\ref{tab:main}. \emph{avg} and
\emph{Local} OOD are defined as in Table~\ref{tab:h60}. Zone A is
clinically accurate. Zones D and E are clinically dangerous (a missed
excursion or the wrong treatment). Zones B and C make up the remainder, and
zone C is below $0.15\%$ in every cell. Bold marks the best value per column
within each block, excluding the centralised baseline.}
\end{center}
\end{table}
...
edge over local training. The same ordering held on the Clarke error grid,
where MLDG also had the fewest clinically dangerous forecasts of the
federated strategies. OOD, MLDG beat the average single-cohort model on
...
four universities, UCL, Manchester, Newcastle, and KCL, training FedAvg,
...
ReplaceBG results vary widely with protocol: one study reports $9.7$ mg/dL
...
Crossformer \cite{zhu2026glullm}   & not stated         & yes & $21.1$ \\
N-BEATS \cite{zhu2024gpformer}     & CGM                & yes & $21.2$ \\
TCN \cite{zhu2026glullm}           & not stated         & yes & $21.3$ \\
...
GRU \cite{zhu2026glullm}           & not stated         & yes & $22.1$ \\
...
\cite{zhu2024gpformer,zhu2026glullm}. Each of those two sources evaluates on a
46-subject hold-out of ReplaceBG, and the GluLLM paper does not state the inputs
of its Crossformer, TCN, and GRU baselines. Our models are evaluated on a 21-patient
...
strategies compare on the forecasting objective they were trained for, so
RMSE is the primary metric and the forecast is a point estimate with no
calibrated uncertainty. The Clarke error grid shows that the ordering holds
on a clinical view of the same forecasts, but the models were not trained
for it, and between $1\%$ and $5\%$ of 30-minute forecasts, depending on the
cohort, still fall in the dangerous zones. The studies we compare against
also report MARD or hypo-/hyperglycaemia detection, which we do not. The
next step is to train with clinical relevance built in, for example a loss
that penalises forecasts falling in the dangerous zones of the error grid,
and to report MARD, event detection, and predictive uncertainty alongside
the grid.
