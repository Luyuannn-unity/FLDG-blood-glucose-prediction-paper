...
range; sustained excursions outside that range drive both the acute dangers
of hypoglycaemia and the long-term microvascular and cardiovascular
complications of hyperglycaemia. Continuous glucose monitoring (CGM), which
reports interstitial glucose roughly every five minutes, is now central to
diabetes care --- it underpins clinical outcomes such as time in range, for
which international consensus targets have been
defined~\cite{battelino2019tir} --- and provides the dense data stream that
makes short-horizon \emph{forecasting} possible.
...
question~\cite{ghimire2024generalize}. Short-horizon CGM forecasting has
progressed from autoregressive and classical machine-learning models to deep
sequence models: recurrent architectures with predictive variance estimation
established strong baselines~\cite{martinsson2020}, standardised datasets
and challenges shaped much of the early evaluation~\cite{marling2020ohio},
and the transformer~\cite{vaswani2017attention} and its long-horizon
time-series variants~\cite{zhou2021informer,wu2021autoformer} have been
adapted to glucose, with personalised, uncertainty-aware transformer
forecasters reporting state-of-the-art
accuracy~\cite{sergazinov2023gluformer} and curated CGM benchmarks
standardising their evaluation~\cite{sergazinov2024glucobench}. But glucose
dynamics, demographics, and devices differ from site to site, and a model
can degrade on populations it did not see in
training~\cite{farahmand2025glumind}. A model that has only seen one cohort
has little reason to generalise to the next; robust generalisation calls for
training over large, diverse populations spanning many sites.
...
clinical trials, device manufacturers, and national registries, and governed
by data-protection regimes such as the GDPR and HIPAA that restrict moving
raw records between institutions, let alone across borders. What is needed
is a large, heterogeneous corpus; what is available is many small, governed
silos. Federated learning (FL) addresses this
tension~\cite{mcmahan2017fedavg}: sites train on their own data and exchange
only model updates, so a shared model is learned without any raw record
leaving its institution, and on this basis FL has been widely adopted for
collaborative medical machine
...
reduction~\cite{karimireddy2020scaffold}. Most of this work, however, is
evaluated in simulation, with the whole federation executed as processes or
dataset shards on a single machine.

Federation removes the need to pool data, but it does not remove domain
shift. Real CGM cohorts differ in device generation, demographics, treatment
protocols, and glycaemic regime, and these differences widen across
countries; a federated model must therefore still generalise to patients,
and even whole cohorts, it never trained on (domain generalisation), and/or
specialise to each site (personalisation). Domain generalisation seeks
models that transfer to unseen distributions~\cite{zhou2022dgsurvey}, and
meta-learning for domain generalisation (MLDG) pursues it by simulating
train/test domain shift within each update, so the model learns to
...
accuracy further~\cite{zhu2026glullm}; but these models are trained on data
pooled at a single site, which is precisely what data-protection regimes
prevent a multi-institution consortium from doing. Our question is what
happens when the same objective must be optimised \emph{without pooling} ---
across several institutions that each hold a different cohort and cannot
exchange raw records --- and how the federated alternatives compare with one
another under that constraint.

Bringing domain generalisation into federation is an active area, surveyed
by Li et al.~\cite{li2025fedDGsurvey}; our benchmark covers two of its
method families, meta-learning and personalisation. Meta-learning approaches
adapt the model-agnostic meta-learning objective to federation, for
communication-efficient convergence~\cite{chen2018fedmeta} or for models
that personalise to a new client in a few gradient
steps~\cite{fallah2020perfedavg}. Personalisation approaches keep a
per-client model: APFL learns an adaptive convex mixture of the global and a
local model~\cite{deng2020apfl}, while Ditto regularises each client's
personal model toward the global one with a tunable proximal
strength~\cite{li2021ditto}. A third family aligns client representations,
via model-contrastive objectives~\cite{li2021moon}, federated contrastive
learning for multi-centre medical imaging~\cite{fedcl2023}, or adversarial
domain alignment~\cite{flda2023,fedadv2022}; we leave it to future work, and
benchmark MLDG, APFL, and Ditto alongside standard federated averaging
...
as clients and executes the whole federation as processes or dataset shards
on a single machine
...
subjects from the UVA/Padova simulator rather than real patient records.
Running a federation for real requires networked, physically separate sites
to exchange updates and to start from a common model; production FL
frameworks can do this, but they carry dependencies and assumptions that can
be ill-suited to a small clinical consortium that simply wants a handful of
sites, one cohort each, to train together. We therefore also ask a
...
strategy rather than model capacity. All cohorts are adult T1D populations, so
our findings should not be assumed to extend to paediatric or type-2 diabetes.
...
leaves per-site accuracy on the table, while specialising to a site forfeits
that transfer. We show it can be resolved by treating the federated model as
a foundation rather than a finished product --- finetuning the global model
on a small fraction of a new cohort's own patients beats training there from
scratch --- so a centre joining after training can reach, and exceed,
local-only performance without a large local data-collection effort, which
is what turns the benchmark into usable deployment advice.
...
Newcastle University, and the University of Oxford --- with one client per
institution, each training only on one dataset and exchanging nothing but
model weights over the public internet. This exercises the conditions a
federation faces across separate administrative domains --- separate
hardware, network round-trips between rounds, no shared filesystem ---
rather than the single-host shortcut that almost all federated glucose work
takes. What we do \emph{not} claim is a production-grade or cross-border
deployment: the system carries no encryption, secure aggregation, or
differential privacy, and all four sites are in one country, so the
additional legal machinery of cross-jurisdiction federation is untested.
Those are engineering and governance gaps, not architectural ones, and we
return to them in the Discussion.
...
federated training and reported separately (Table~\ref{tab:ood}). ARISES is both the
...
the four clients (step 1); each client trains them for one local epoch on its
own cohort (step 2) and returns the updated weights (step 3); the server
...
client starts from identical parameters. All traffic crosses the public
internet over plain HTTP, and raw CGM data never leaves its institution.
...
\textbf{FedAvg / FedProx.} Standard federated averaging
(FedAvg)~\cite{mcmahan2017fedavg}, and FedProx~\cite{li2020fedprox}, which adds
...
adapted to the federated setting. At each client step, the batch is split by
...
patients would fall back to a plain gradient step; this never happened in any
of the $191{,}100$ MLDG training steps across the five seeds). Writing
...
dependence of $\tilde{w}$ on $w$ (second order; no first-order approximation)
...
\textbf{Personalised FL (PFL).} APFL~\cite{deng2020apfl} learns a per-client
...
a decoupled APFL variant. Ditto~\cite{li2021ditto} trains a personal model
pulled toward the global one by a proximal term ($\mu\in\{0.01,0.1,1.0\}$). These methods produce one personalised
...
accuracy at the contributing sites. \emph{OOD} performance is measured on the
three held-out cohorts (ReplaceBG, BrisT1D, Flair), isolating cross-domain transfer; only models that can be deployed as a
...
unseen in the strong sense --- unseen cohort \emph{and} unseen individual. Each held-out cohort is split by patient in
...
random seeds. We report the mean and standard deviation across seeds. To test
whether the small differences between federated strategies are meaningful, we
compare per-seed RMSE with a paired $t$-test across seeds.
...
as a local baseline. \emph{Finetuned} starts from a trained federated global
model (FedAvg, FedProx, or MLDG) and continues training --- all parameters,
$10$ epochs, learning rate $10^{-4}$ --- on the target's training patients,
...
% data — "exempt, and why" at minimum). See reviews/REQUIRED_FROM_YOU.md
% items 6-7.
...
\paragraph{MLDG has the lowest error in every reported comparison, within seed
noise.} Among the federated strategies that produce a deployable global
...
--- FedProx on ReplaceBG, MLDG on BrisT1D and
Flair --- but the three cluster within $\approx 0.1$ mg/dL, so the practical
message is robust to that choice: pretraining across cohorts helps, and the
aggregation rule matters less once local data are available. Both arms see
identical target data, so the gain is attributable to the federated
...
\hline
train from scratch (100\% local) & $21.38 \pm 0.09$ & $26.25 \pm 0.33$ & $24.50 \pm 0.11$ \\
\hline
FedAvg $\rightarrow$ finetune  & $21.04 \pm 0.04$ & $25.68 \pm 0.15$ & $24.31 \pm 0.06$ \\
FedProx $\rightarrow$ finetune & $\mathbf{20.98 \pm 0.06}$ & $25.64 \pm 0.13$ & $24.32 \pm 0.08$ \\
MLDG $\rightarrow$ finetune    & $21.04 \pm 0.07$ & $\mathbf{25.57 \pm 0.09}$ & $\mathbf{24.21 \pm 0.06}$ \\
\thickhline
\end{tabular}
\begin{center}
...
zero-shot was weakest (BrisT1D $26.31\rightarrow25.57$, Flair
$25.05\rightarrow24.21$; cf.\ Table~\ref{tab:ood}).}
\end{center}
\end{table}
...
mg/dL at 30 minutes) and came within $0.09$ mg/dL of a convergence-matched
centralised model trained on the four cohorts pooled ($19.90$) --- so the
accuracy cost of never pooling raw data was smaller than the error of the
sensors the model reads from. OOD, the choice of a single training cohort
decided everything: the best single-cohort model matched the federated models,
the smallest-cohort model collapsed on all three held-out cohorts, and
in-domain accuracy did not tell the two apart. Every federated model avoided
that collapse. These results were produced by a federation that ran
...
finetunes best depends on the cohort (FedProx on ReplaceBG, MLDG on BrisT1D and
Flair), but the three end within $\approx 0.1$ mg/dL of each other. So the
...
\paragraph{On meta-learning.} MLDG's edge is small, consistent, and
underpowered (Limitations (iii)), but its mechanism is worth a note: our MLDG
splits patients into meta-train and meta-test \emph{within} each client, so it
