...
more representative. Taken further, that points to a foundation model for
CGM. General time-series foundation models are already being benchmarked on
glucose forecasting \cite{lu2026glucofmbench}; building one specific to CGM
would need data from many sites, which today means pooling it. Federation is
a way to train on that data without moving any of it. Our four-cohort adult
T1D model is far short of that scale, and we ran no experiment on how many
sites such a model would need. What we report is a prerequisite --- a shared
model a new site can adapt cheaply --- not evidence that the larger model
would work.
