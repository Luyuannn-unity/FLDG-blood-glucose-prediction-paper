...
Taiyu Zhu\textsuperscript{3},
Simon Harper\textsuperscript{4},
...
\\
\textbf{4} University of Manchester, Manchester, United Kingdom
\\
\bigskip

* ken.li@ucl.ac.uk
% NOTE (author): (a) confirm name order for "Rui Sun" (given name first, per
% PLOS "First name Last name" convention; the input read "Sun Rui");
% (b) fill affiliations 2 and 3; (c) add department for Manchester if wanted;
% (d) PLOS wants each author's ORCID (corresponding author's is mandatory) and a
...
separate accuracy on the training cohorts (held-in) from accuracy on unseen
cohorts (out-of-distribution, OOD), and almost all such work is simulated.
Using one encoder--decoder transformer, four real T1D cohorts as federated
clients and three never-seen cohorts (ReplaceBG, BrisT1D, Flair) as OOD
tests, we benchmarked single-cohort local training against federated
...
Federation beat local training on the contributing cohorts. MLDG
reached $20.25$ mg/dL RMSE at 30 minutes against $20.62$, lower on every
seed, and beat FedAvg by $0.18$ mg/dL, as much as FedAvg beat local
training. The privacy constraint proved nearly free: MLDG came within $0.09$ mg/dL of a
model trained on the four cohorts pooled, under $1\%$ of the $12$--$14$
mg/dL error of CGM sensors, and tied it at 60 minutes.
Out of distribution, MLDG was the best federated model on all three unseen
cohorts. On ReplaceBG it reached
$21.30$ mg/dL without seeing any of its patients, within the range of models
trained on it. Finetuning the federated model on a new site's
patients beat training there from scratch on all three cohorts, and $10$ to
$17$ patients sufficed on the two larger ones. We ran the federation live
across four universities, not in simulation, with tooling light enough for
small clinics.
...
training data. The shared model beat or matched each site training alone on
its own patients. Federation cost almost nothing: it came within $0.09$ mg/dL
...
of its own patients where the cohort is large, and about a third where it is
small.

We ran this across four universities for real, each holding only its own data.
That gives a small group of clinics a practical route to better predictors.
...
readings in the clinical 70--180 mg/dL band. For HUPA-UCM and T1D-UOM the
...
dataset & patients & total windows & mean CGM & sd CGM & TIR (\%) \\
        &          &               & (mg/dL)  & (mg/dL) & 70--180  \\
\hline
HUPA-UCM & 22 & 205,849 & 139.3 & 55.1 & 74.0 \\
ABC4D    & 25 & 562,043 & 155.1 & 63.4 & 65.5 \\
ARISES   & 12 & 126,248 & 160.5 & 63.7 & 63.7 \\
T1D-UOM  & 14 & 290,753 & 151.2 & 58.7 & 70.7 \\
\thickhline
\end{tabular}
\begin{center}
...
the totals in the original dataset publications, and the window counts and
statistics are after the cleaning step described in the text.}
\end{center}
\end{table}
...
All models, including the single-cohort models, are trained with this shared
constant.
% NOTE (author, 2026-08-24): the reporting evaluation (eval_extended.py)
% recomputes the normalisation constants from each arm's training directories
% and overrides the config: the pooled clean four-cohort statistics for the
% federated and centralised arms (152.18 / 61.09, immaterial) and each single
% cohort's own statistics for the single-cohort arms (e.g. HUPA-UCM 136.29 /
% 52.36, which shifts that model's held-in RMSE by about 0.5 mg/dL at seed 42:
% 20.44 with the training constants vs 20.99 reported). Decision pending: rerun
% the extended evaluation of the four single-cohort arms with the training
% constants, or describe the mismatch here. See REQUIRED_FROM_YOU.md item 16.
...
FedAvg, FedProx, MLDG, the centralised reference, and each single-cohort
...
\hline
Local (per cohort) & $20.84 \pm 0.25$ & $19.75 \pm 0.11$ & $\mathbf{22.06 \pm 0.41}$ & $19.82 \pm 0.18$ & $20.62 \pm 0.09$ \\
\hline
...
MLDG    & $\mathbf{19.84 \pm 0.13}$ & $\mathbf{19.53 \pm 0.08}$ & $\mathbf{22.06 \pm 0.11}$ & $\mathbf{19.58 \pm 0.08}$ & $\mathbf{20.25 \pm 0.08}$ \\
\hline
...
(see text). A tie is marked on both rows.}
\end{center}
\end{table}

\paragraph{Federation beats local training held-in.} On the contributing
cohorts, the global MLDG and FedAvg models are more accurate than
cohort-specific local training (Table~\ref{tab:main}). On the held-in average,
MLDG is $0.36$ mg/dL below local training ($20.25$ vs $20.62$), lower on all
five seeds (paired $t$-test, $p = 0.005$), and FedAvg is $0.18$ mg/dL below
($p = 0.015$, 5 of 5 seeds). FedProx does not differ from local training at
this horizon ($p = 0.77$). The gain is not spread evenly. It is largest on
HUPA-UCM ($1.00$ mg/dL for MLDG, $p = 0.002$), present on ABC4D ($0.22$,
$p = 0.005$) and T1D-UOM ($0.24$, $p = 0.02$), and absent on ARISES, where
local training and MLDG tie at $22.06$ mg/dL. Without HUPA-UCM the average
MLDG gain is about $0.15$ mg/dL. So pooling four heterogeneous cohorts
through federation improves in-domain accuracy, by a small but consistent
margin, even though each client sees only the shared global weights.

\paragraph{Out of distribution, MLDG is the best federated model and
federation beats the average single cohort.} We test transfer on three
...
BrisT1D, $0.09$ on Flair) and level with the centralised reference. The four
single-cohort models are all stable across seeds (sd $0.09$ to $0.21$). MLDG
and FedAvg beat their mean on every target (MLDG by $0.3$ to $0.6$ mg/dL, on
all five seeds), and FedProx matches or beats it. These gains are small next to the measurement error of current CGM
...
even edges MLDG by $0.1$ to $0.2$ mg/dL. The HUPA-UCM model transfers worst,
$1.2$ to $1.6$ mg/dL behind MLDG on every target. A
...
\;trained on HUPA-UCM & $22.58 \pm 0.21$ & $27.85 \pm 0.20$ & $26.23 \pm 0.21$ \\
\;trained on ABC4D    & $21.47 \pm 0.13$ & $26.42 \pm 0.16$ & $25.16 \pm 0.14$ \\
\;trained on ARISES   & $21.71 \pm 0.19$ & $26.16 \pm 0.17$ & $24.84 \pm 0.09$ \\
\;trained on T1D-UOM  & $21.83 \pm 0.13$ & $26.56 \pm 0.14$ & $25.11 \pm 0.13$ \\
Local (mean of the four) & $21.90 \pm 0.06$ & $26.75 \pm 0.06$ & $25.34 \pm 0.04$ \\
\thickhline
\end{tabular}
\begin{center}
...
federated strategies. The centralised reference lands within $0.12$ mg/dL of
...
survives. Federation still beats local training held-in, now for all three
global strategies (MLDG by $0.60$ mg/dL, $p = 0.0001$, FedAvg by $0.39$,
$p = 0.001$, FedProx by $0.31$, $p = 0.02$). Ditto still sits at FedAvg's
...
$0.5$ mg/dL, $p \le 0.024$). The centralised reference ties MLDG exactly on
...
model on every unseen cohort, on all five seeds, and FedAvg stays at or below
it.
...
\hline
Local (per cohort) & $36.76 \pm 0.39$ & $33.86 \pm 0.19$ & $36.26 \pm 0.55$ & $32.28 \pm 0.13$ & $34.79 \pm 0.03$ & $37.21 \pm 0.03$ & $44.71 \pm 0.11$ & $41.10 \pm 0.06$ \\
\hline
...
\hline
APFL           & $35.77 \pm 0.17$ & $34.04 \pm 0.31$ & $\mathbf{36.07 \pm 0.10}$ & $32.20 \pm 0.08$ & $34.52 \pm 0.11$ & --- & --- & --- \\
...
\hline
\multicolumn{9}{l}{\emph{centralised reference (pools all four cohorts' raw data)}} \\
...
reference pools raw data and is unavailable in our setting. Bold marks the
lowest value per column excluding the centralised reference. The 60-minute
...
absolute terms but consistent, and at 30 minutes it is as large as the gain
of FedAvg over local training ($0.18$ mg/dL in both cases). FedAvg and FedProx
...
averages sit at FedAvg's level, plain APFL's is above local training's, and
...
the two are level. MLDG is within $0.12$ mg/dL of the pooled model on all
...
both zero-shot MLDG and the pooled centralised model are behind a model
...
there ($21.30$ vs $21.45$ mg/dL, $p = 0.018$) and the pooled model is level
...
OOD). Federation beat single-cohort local training in-domain and transferred
about as well as the best single cohort out of it, and better than the
average one. Held-in, the global MLDG and FedAvg models were more accurate
than local training (average RMSE $20.25$ and $20.43$ vs.\ $20.62$ mg/dL at
30 minutes, both on five of five seeds), and MLDG came within $0.09$ mg/dL of
...
held-in and OOD comparison, and at 30 minutes its edge over FedAvg was as
large as FedAvg's edge over local training. OOD, MLDG and FedAvg beat the
average single-cohort model on every held-out cohort and FedProx matched it,
and a cohort's in-domain accuracy did not predict how well its model
transferred. These results were produced by a federation that ran across
...
\paragraph{Federation gains in-domain and removes the need to pick a source
cohort.} Federation's first benefit is in-domain accuracy. MLDG is $0.36$
mg/dL below local training on the held-in average and FedAvg $0.18$, both on
every seed. The gain is largest on HUPA-UCM, present on ABC4D and T1D-UOM,
and absent on ARISES, so it is small in absolute terms and uneven across
sites, but it is consistent. The second benefit is out of distribution. The
four single-cohort models transfer differently, with a spread of $1.1$ to
$1.7$ mg/dL between the best and the worst on each held-out cohort, and
...
site's model cannot know in advance which it is getting. MLDG and FedAvg beat
the average single-cohort model on all three independent held-out cohorts,
and FedProx matched it, a replicated pattern, not a one-off. This is not a guarantee (three held-out cohorts do
...
and hardest cohort, where local training only tied MLDG. These methods also
...
minutes the choice of strategy mattered as much as federating at all. MLDG
gained $0.18$ mg/dL over FedAvg, FedAvg gained $0.18$ over local training,
and FedProx gained nothing. The data still dominate, since the centralised
model ties MLDG, but the training rule is not a detail.

\paragraph{Implications for a small group of clinics.} The case for
federating is this: a centre gains accuracy on its own patients and gets a
model that transfers to new populations without a guess about which source
cohort to trust. And since these results were themselves produced by a
...
explicit, and an informative one. In-domain, federation beats local training
and the meta-learned strategy beats plain averaging. OOD, federation transfers
...
significant are small in absolute terms (mostly $0.2$ to $0.6$ mg/dL, and
$1.0$ mg/dL on one cohort), and the held-in gain over local training is
concentrated on that cohort (HUPA-UCM).
Some comparisons remain unsettled at this sample size: MLDG against the
centralised reference, MLDG against FedAvg on Flair at 30 minutes, and
FedAvg against FedProx. FedProx used a single untuned $\mu$. (APFL, by
...
\emph{(vii) Mechanism.} The centralised reference shows that federation
