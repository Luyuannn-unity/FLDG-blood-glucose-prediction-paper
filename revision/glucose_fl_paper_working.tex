...
single machine
\cite{piao2023graph,piao2025gluadfl,dave2025fedglu,defalco2023federated}. The
...
\cite{sun2024multicontinental}, who deploy a blockchain-enabled federated glucose
...
on the dataset that institution was responsible for and exchanging nothing but
model weights over the public internet. We do not claim that each site
originated its cohort: two of the four are custodians of the clinical study that
produced theirs, while the others host a dataset released publicly. What the run
exercises is the conditions a federation faces across separate administrative
domains --- separate hardware, network round-trips between rounds, no shared
filesystem --- rather than the single-host shortcut that almost all federated
glucose work takes. What we do
...
The closest prior work, Sun et al.\ \cite{sun2024multicontinental}, is also
genuinely distributed --- four PCs and a server across five cities on three
continents --- but its clients hold in-silico subjects from the UVA/Padova
simulator, so no real patient record enters that federation.
