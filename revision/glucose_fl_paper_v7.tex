...
generalisation is the right objective for cross-population glucose forecasting,
and we adopt both its architecture and its MLDG objective rather than reinventing
them. What it does \emph{not} address is the constraint that motivates this
paper: GPFormer is trained centrally, on pooled data from one cohort, which is
precisely what data-protection regimes prevent a multi-institution consortium
from doing. Our question is therefore what happens when that objective must be
...
The forecaster is GPFormer \cite{zhu2024gpformer}, a sequence-to-sequence
(encoder--decoder) Transformer for glucose prediction that maps the 72-step CGM
context to the 12-step horizon. We adopt its architecture unchanged so that our
comparison isolates the \emph{training} strategy rather than the model. It uses
...
features we do not use) and GPFormer itself ($19.2$) --- the same architecture we
train, but trained centrally on ReplaceBG rather than federated across four other
cohorts. The $2.1$ mg/dL gap to GPFormer is, in effect, the price of never seeing
the target cohort and never pooling raw data, and it is smaller than the spread
among the in-distribution baselines themselves. Two honest caveats: the published
studies evaluate on a 46-subject hold-out from within ReplaceBG whereas every
ReplaceBG patient is unseen to us, so the test sets are not identical (ours is
arguably the harder setting, but it is not the same one); and reported ReplaceBG
results vary widely with protocol --- one study reports $9.3$ mg/dL
\cite{jaloli2023cnnlstm}, but on a 15-minute grid and with a rolling split that
re-admits test patients to training, which is not comparable to any of the numbers
here.
...
by-patient hold-out on a 5-minute grid and report a point error at 30 minutes.
Every published row was \emph{trained on ReplaceBG}; our three rows were trained
on four entirely different cohorts and never saw ReplaceBG, so they are zero-shot.
Published values are read from \cite{zhu2024gpformer,zhu2026glullm}; the two
sources evaluate on the same 46-subject hold-out, while our models are evaluated
on ReplaceBG patients all of whom are unseen.}
\end{center}
\end{table}
