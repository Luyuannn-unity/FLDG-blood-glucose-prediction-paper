...
from scratch on the same data, by $0.3$--$0.7$ mg/dL. The best finetuned model
edges the from-scratch baseline by $0.40$ mg/dL on ReplaceBG
($20.98$ vs $21.38$), $0.29$ on Flair ($24.21$ vs $24.50$), and $0.68$ on BrisT1D
($25.57$ vs $26.25$). Which federated strategy finetunes best differs by cohort
--- FedProx on ReplaceBG, MLDG on the two truly held-out cohorts (Flair and
...
experiments make the ``foundation-then-adapt'' recipe concrete, and --- unlike in
our earlier analysis of a single cohort --- it now holds across all three held-out
cohorts (Table~\ref{tab:finetune}). On every target, finetuning a federated model
